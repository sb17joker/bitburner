...
import TIXStock from "if.stock.notix";
import WSEStock from "if.stock.wse";
import { getStocks } from "if.stock.wse";
...
export async function main(ns) {
    ...
    if (( permissions & ACL.WSE ) && !(permissions & ACL.TIX) && !(permissions & ACL.TIX4S) && !(permissions & ACL.WSE4S)) { await getStocks(ns) };
    ...
    for (let s of stocks) {
        //await s.updateCache(false); //commented out due to error, null
    }

    if (!(permissions & ACL.WSE)) { return }
    
    if (permissions & ACL.WSE) {
        stocks = [];
        for (let s of stock_list) {
    ...
    if ((permissions & ACL.TIX4S) && (permissions & ACL.WSE4S)) {
        stocks = [];
        for (let s of stock_list) {
    ...
    while (true) {
        console.log(stocks);
        if ((permissions & ACL.WSE) && !(permissions & ACL.TIX) && !(permissions & ACL.TIX4S) && !(permissions & ACL.WSE4S)) { getStocks() };
